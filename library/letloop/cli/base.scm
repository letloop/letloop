             ...
             (call-with-values (lambda () (guess (car standalone)))
               (lambda (type string*)
                 (case type
                   ...
                   (file (errors (format #f "Dubious file: ~a" string*)))
         ...
         (set! program.scm
              (call-with-output-file "/tmp/letloop.scm"
                (lambda (port)
                  ...
                  (write `(scheme-start ,main port)) 'truncate)
              "/tmp/letloop.scm"))

         (pk 'compiling (maybe-compile-file (pk 'program.scm program.scm))))

       (pk 'make-boot-file)
       (apply make-boot-file (pk (string-append (if program.scm
                                              (basename-without-extension program.scm)
                                              "./library")
                                            ".boot"))
              (list "scheme")
              (append (filter file-exists? (map .so (map car (letloop-discover-libraries))))
